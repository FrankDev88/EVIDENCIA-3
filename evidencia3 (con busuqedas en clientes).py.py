...
def mostrar_clientes(conn):
    try:
            cur = conn.cursor()
            cur.execute("SELECT id_clientes, nombre, RFC, CORREO FROM CLIENTES")
...
def mostrar_servicios(conn):
    try:
        cur = conn.cursor()
        cur.execute("SELECT id_servicio, nombre, costo FROM SERVICIOS")
                ...
                try:
                    while True:
                        try:
                            ...
                            cur.execute("SELECT id_clientes FROM CLIENTES WHERE id_clientes = :id_cliente", valores)
                        ...
                        while True:
                            ...
                            while True:
                                try:
                                    ...
                                    cur.execute("SELECT id_servicio FROM SERVICIOS WHERE id_servicio = :id_servicio", valores)
                                    bandera = cur.fetchone()
                                    if bandera:
...
def registrar_cliente(conn):
    try:
        ...
        cur.execute("INSERT INTO CLIENTES (nombre, RFC, CORREO) VALUES (:nombre, :rfc, :correo)", valores)
        conn.commit()
...
def reporte_clientes_activos_por_nombre(conn):
    try:
        cur = conn.cursor()
        cur.execute("SELECT id_clientes, nombre, RFC, CORREO FROM CLIENTES ORDER BY nombre")
        clientes = cur.fetchall()
...
def agregar_servicio(conn):
    try:
        ...
        cur.execute("INSERT INTO SERVICIOS (nombre, costo) VALUES (?, ?)", (nombre_servicio, costo_servicio))
        conn.commit()
...
def buscar_servicio_por_nombre(conn):
    try:
        ...
        cur.execute("SELECT id_servicio, nombre, costo FROM SERVICIOS WHERE UPPER(nombre) = UPPER(?)", (nombre_buscar,))
...
def buscar_servicio_por_clave(conn):
    try:
        cur = conn.cursor()
        cur.execute("SELECT id_servicio, nombre, costo FROM SERVICIOS ORDER BY id_servicio")
        servicios = cur.fetchall()
...
def generar_reporte_servicios_por_nombre(conn):
    try:
        cur = conn.cursor()
        cur.execute("SELECT id_servicio, nombre, costo FROM SERVICIOS ORDER BY nombre")
        servicios = cur.fetchall()
        if not servicios:
            print("No hay servicios registrados.")
            return
...
def buscar_clientes_por_nombre(conn):
    try:
        ...
        nombre = nombre.lower()
        cur = conn.cursor()
        cur.execute("SELECT id_clientes, nombre, RFC, CORREO FROM CLIENTES WHERE lower(nombre) LIKE :nombre", {"nombre": f"%{nombre}%"})
        clientes = cur.fetchall()
        ...
        while True:
            try:
                ...
                if clave == 0:
                    return
                cur = conn.cursor()
                cur.execute("SELECT id_clientes, nombre, RFC, CORREO FROM CLIENTES WHERE id_clientes = :clave", {"clave": clave})
          ...
          while True:
            try:
              ...
              if MENU_CLIENTES == 2:
                while True:
                  try:
                     MENU_CONSULTAS = int(input("------- ESTA EN EL MENU CONSULTAS A QUE AREA DESEA IR -------\n1)MENU DE CONSULTAS OREDENADAS\n2)BUSQUEDA DE CLIENTES POR CLAVE(NO DISPONIBLE)\n3)BUSQUEDA DE CLIENTES POR NOMBRE(NO DISPONIBLE)\n4)SALIR\n"))
                     if MENU_CONSULTAS == 1:
                      while True:
                        try:
